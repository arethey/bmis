...
if(isset($_POST['btn_add'])){
    ...
    $txt_Clearance = $_POST['txt_Clearance'];
    ...
    if($num_row > 0)
    {
        ...
        while($row = mysqli_fetch_array($chk)){
            ...
            if($row['lengthofstay'] < 6){
                ...
                $txt_Clearance = $_POST['txt_Clearance'];
                $date = date('Y-m-d');
                $reqquery = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,residentid,findings,Clearance,orNo,samount,dateRecorded,recordedBy,status) 
                    values ('','".$_SESSION['userid']."','','$txt_Clearance','','','$date','".$_SESSION['role']."','New') ")or die('Error: ' . mysqli_error($con));
...
if(isset($_POST['btn_save']))
{
    ...
    $txt_edit_Clearance = $_POST['txt_edit_Clearance'];
    $txt_edit_ornum = $_POST['txt_edit_ornum'];
    $txt_edit_amount = $_POST['txt_edit_amount'];

    $update_query = mysqli_query($con,"UPDATE tblclearance set clearanceNo= '".$txt_edit_cnum."', findings = '".$txt_edit_findings."', Clearance = '".$txt_edit_Clearance."', orNo = '".$txt_edit_ornum."', samount = '".$txt_edit_amount."' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
